  ...
  // 周期延拓
   ...
   /////////////////////////////////////////////////////////             
    ...
    for ( i = a-1; i < a + NN-1; i++ )
        ...
        if (*sigma <= 10 && meand > (*sigma)*(*sigma)*0.000174*0.95)      
           continue; 
        if ( *sigma > 10 && *sigma <= 20 && meand > (*sigma)*(*sigma)*0.000174*0.95 )
           continue; 
         if ( *sigma > 20 && *sigma <= 50 && meand > (*sigma)*(*sigma)*0.000174*1.0 )
           continue;                      
         if ( *sigma > 50 && *sigma <= 75 && meand > (*sigma)*(*sigma)*0.000174*1.05 )
           continue;                       
        if ( *sigma > 75 && meand > (*sigma)*(*sigma)*0.000174*1.05 )
           continue;                        
   /////////////////////////////////////////////////////////////             
...
//           printf("%d,%d,%d,%d\n",dis3[0][0],dis3[0][1],dis3[5][9],dis3[10][11]); 
 ...
 for (k = 0; k < N12*N12; k++)    //开始循环
     ...
     for ( i = 0; i < N32; i++ )
        ...
        float sum = 0.0, beta = 0.0; 
  
       /*
        if ( *sigma < 10 )
            beta = 0.4;
        else if ( *sigma >= 10 && *sigma < 20)
            beta = 0.75;
        else if ( *sigma >= 20 && *sigma < 30)
            beta = 0.8;
        else if ( *sigma >= 30 && *sigma < 40)
            beta = 0.85;
        else if ( *sigma >= 40 && *sigma < 70)
            beta = 0.9;
        else
            beta = 1.05;
         */  
...
/////////////////////////////////////////////////////////////////////
           if ( meand <= (*sigma)*(*sigma)*0.000174*1.07 )
              {
             sum += dis4[IndexArrays[1]];
             sum = sqrt(sum/N22);
             *sumd += sum;
              h++;
             }
